...
Rules of the game: `app/public/rules-fr.pdf` (and `rules-en.pdf`). Every place where this adaptation
does something the rulebook does not say, and every ambiguity that had to be settled, is listed in
`docs/ecarts-regles.md` — read it before "fixing" a rule that looks wrong, and add to it when you
settle a new one.
...
```typescript
export const GreyluneOptionsSpecV2: OptionsSpecV2 = {
  specVersion: 2,
  players: { min: 2, max: 4 },
