...
import { tableInside } from './TableLayout'
 ...
 * left column of the Village, the top card of a column of Objects, the Encounter rows running above and
 * below the main board. It is pushed back in by as much as it spills over, and not moved at all when it
 * fits.
 *
...
export const cardHoverTransform = (item: MaterialItem<PlayerColor, LocationType>, context: ItemContext<PlayerColor, MaterialType, LocationType>): string[] => {
  ...
  const dx = pushInside(x, (width * hoverScale) / 2, tableInside.xMin, tableInside.xMax)
  const dy = pushInside(y, (height * hoverScale) / 2, tableInside.yMin, tableInside.yMax)
