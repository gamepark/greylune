import { force, magic, vp } from '@gamepark/greylune/material/Effect'
 ...
 *
 * A marker already at 5 offers the victory points its track pays instead, on the laurel the board
 * prints above the top of the track.
 */
export const ChooseSkillMenu = ({ marker, move, count, full }: { marker: SkillMarker; move: MaterialMove; count: number; full: boolean }) => {
  ...
  return (
    <GreyluneMenuButton
      {...skillMarkerMenuSpot}
      move={move}
      label={t(full ? 'action.gain-vp' : isForce ? 'action.gain-force' : 'action.gain-magic', { count })}
      labelPosition={isForce ? 'left' : 'right'}
    >
      <GainLabel gain={full ? vp(count) : isForce ? force(count) : magic(count)} />
    </GreyluneMenuButton>
  )
}
